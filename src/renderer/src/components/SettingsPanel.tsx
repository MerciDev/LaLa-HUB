...
function SettingsPanel({ visible, onClose, onJumpToHeader, gridConfig, onGridConfigChange, onClearGrid, initialTab }: SettingsPanelProps): React.JSX.Element {
    ...
    const [friendsList, setFriendsList]     = useState<import('../../../shared/types').FriendProfile[]>([])
    const [friendSearchQuery, setFriendSearchQuery] = useState('')
    const [friendSearchResults, setFriendSearchResults] = useState<import('../../../shared/types').UserProfile[]>([])
    const [isSearchingFriends, setIsSearchingFriends] = useState(false)
    const [friendSearchMessage, setFriendSearchMessage] = useState<string | null>(null)
    const [socialNotification, setSocialNotification] = useState<{ message: string; type: 'success' | 'error' | 'info' } | null>(null)
        ...
        >
            ...
            {tab === 'friends' && (
                <div className="cp-section">
                    {/* glow */}
                    {/* icon */}
                    <form onSubmit={handleSearchFriends} className="cp-form" style={{ background: 'transparent', border: 'none', padding: 0, marginBottom: '24px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <input
                                ...
                                onChange={e => { setFriendSearchQuery(e.target.value); setFriendSearchMessage(null) }}
                            ...
                            />
                            <button type="submit" disabled={isSearchingFriends} style={{ padding: '10px 16px', borderRadius: '8px', background: 'var(--accent-color, #e60012)', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
                            ...
                            <ul className="cp-list">
                                {friendSearchResults.map(user => (
                                    <li key={user.id} className="cp-list-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', marginBottom: '8px' }}>
                                        ...
                                        <button 
                                            disabled={friendsList.some(f => f.id === user.id)}
                                            onClick={() => handleSendFriendRequest(user.id)} 
                                            style={{ padding: '6px 12px', borderRadius: '6px', background: friendsList.some(f => f.id === user.id) ? 'transparent' : 'rgba(255,255,255,0.1)', color: friendsList.some(f => f.id === user.id) ? 'rgba(255,255,255,0.5)' : '#fff', border: 'none', cursor: friendsList.some(f => f.id === user.id) ? 'default' : 'pointer', fontSize: '13px' }}
                                        >
                                            {friendsList.find(f => f.id === user.id)?.friendshipStatus === 'accepted' ? 'Amigos' : friendsList.some(f => f.id === user.id) ? 'Pendiente' : 'Enviar solicitud'}
                                        </button>
                                    </li>
                                ))}
                            </ul>
                            ...
                            <ul className="cp-list">
                                {friendsList.filter(f => f.friendshipStatus === 'pending').map(req => (
                                    <li key={req.id} className="cp-list-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', marginBottom: '8px' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '8px' }}>
                                            {!req.isSender && (
                                                <button onClick={() => handleAcceptFriendRequest(req.id)} style={{ padding: '6px 12px', borderRadius: '6px', background: '#2ec4b6', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
                                                    Aceptar
                                                </button>
                                            )}
                                            <button onClick={() => handleRemoveFriend(req.id)} style={{ padding: '6px 12px', borderRadius: '6px', background: 'rgba(255,255,255,0.1)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '13px' }}>
                                                Rechazar
                                            </button>
                                        </div>
                                    </li>
                                    ...
                                    <li key={friend.id} className="cp-list-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', marginBottom: '8px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            ...
                                            <div>
                                                ...
                                                <div className="cp-list__item-sub" style={{ fontSize: '12px', color: friend.status === 'in-game' ? '#2ec4b6' : 'rgba(255,255,255,0.5)' }}>
                                                    {friend.status === 'in-game' ? `Jugando a ${friend.currentGame || 'un juego'}` : friend.status === 'online' ? 'En línea' : 'Desconectado'}
                                                </div>
                                            </div>
                                        </div>
                                        <button className="cp-icon-btn cp-icon-btn--danger" title="Eliminar amigo" onClick={() => handleRemoveFriend(friend.id)}>
                                            <Icon icon="mynaui:trash" />
                                        </button>
                                    </li>
                                ))}
